...
date: 2026-08-15T13:32:59-05:00
author: "Jason, Cyber Professional"
section: "scripts"
tags: ["powershell", "troubleshooting", "dism", "sfc"]
description: "Automates the DISM + SFC Windows system file repair workflow — checks for admin rights, runs the repair sequence, logs everything, and reports whether corruption was found and fixed."
github: "https://github.com/thestrad031487/cybergrind-scripts/blob/main/Troubleshooting/Repair-WindowsSystemFiles.ps1"
---

## What it does

Automates the standard Windows corruption repair workflow: runs DISM to repair the component store, then runs SFC to scan and repair corrupted system files. Checks for admin rights before running, logs every step with timestamps, and gives a plain-language summary of whether corruption was found and fixed — no scrolling through raw console output to figure out what happened.

## Requirements

- Windows 10 or Windows 11
- Must be run from an elevated (Administrator) PowerShell session
- Internet access for DISM to pull repair files via Windows Update (unless using -Source with local media)

## Usage

.\Repair-WindowsSystemFiles.ps1

.\Repair-WindowsSystemFiles.ps1 -Source D:\
...
DISM can sit at 50-70% progress for a long time — this is normal, don't close the window. If Windows Update is broken, DISM will fail without a -Source pointing at installation media or a mounted image. SFC results and any unrepaired corruption details are also written to C:\Windows\Logs\CBS\CBS.log.

### References

- [View on GitHub](https://github.com/thestrad031487/cybergrind-scripts/blob/main/Troubleshooting/Repair-WindowsSystemFiles.ps1)
